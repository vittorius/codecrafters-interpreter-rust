use std::{
    cell::{RefCell, RefMut}, fmt::Display, mem, rc::Rc,
};
...
// impl<'i> Interpreter<'i> {
    ...
    pub fn interpret_expr(&mut self, expr: &Expr<'_>) -> EvalResult {
        let mut env = Environment::new();
        self.evaluate(expr, &mut env).map(|v| v.to_string())
    }

    fn execute<'a>(&mut self, stmt: &'a Stmt<'a>, env: Rc<RefCell<Environment<'_>>>) -> StmtResult {
    ...
    fn evaluate(&mut self, expr: &Expr<'_>, env: Rc) -> ExprResult {
        expr.accept(self)
    }
    ...
    fn visit_variable<'t>(&self, name: &'t Token<'_>, env: &'t mut Environment<'t>) -> ExprResult {
...
impl<'e> expr::VisitorMut<'e, ExprResult> for Interpreter {
    fn visit_expr(&mut self, expr: &'e Expr<'e>, env: &'e mut Environment<'e>) -> ExprResult {
        match expr {
            Expr::Binary {
                left,
                operator,
...
impl stmt::VisitorMut<StmtResult> for Interpreter {
    fn visit_stmt<'a>(
        ...
        match stmt {
            Stmt::Expression(expr) => self.evaluate(expr, &mut **env).map(|_| VOID),
            Stmt::Print(expr) => {
            ...
            Stmt::Block(statements) => {
                self.execute_block(
                    statements,
                    Environment::with_enclosing(Rc::clone(&env)),
